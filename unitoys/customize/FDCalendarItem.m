...
- (instancetype)init {
    if (self = [super init]) {
//        [[NSCalendar currentCalendar] setTimeZone:[NSTimeZone localTimeZone]];//设置当前时区，防止与系统的时区不同，会造成比较有误差
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        if ((day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date])&&(self.tag==999)) {
        ...
        //如果不是记录的运动日期则显示灰色
        //        NSLog(@"时间cuo:%@",[NSString stringWithFormat:@"%d",(int)[[self dateOfMonth:FDCalendarMonthCurrent WithDay:day] timeIntervalSince1970]]);
        
        NSString *allDate = [self.arrDisableDate componentsJoinedByString:@","];
        if ([allDate containsString:[NSString stringWithFormat:@"%d",(int)[[self dateOfMonth:FDCalendarMonthCurrent WithDay:day] timeIntervalSince1970]]]) {
            cell.dayLabel.textColor = [UIColor blackColor];
            ...
            cell.chineseDayLabel.textColor = [UIColor lightGrayColor];
        }
        ...
        if ([[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth] && ![[NSCalendar currentCalendar] isDateInToday:self.date]) {
            ...
            // 将当前日期的那天高亮显示
            if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:[NSDate date]]) {  //如何做到选中的日期显示红色
...
- (BOOL)isThisMonth :(NSDate *)date{
    ...
    NSInteger selfMonth = [calendar component:NSCalendarUnitMonth fromDate:date];
    NSInteger newMonth = [calendar component:NSCalendarUnitMonth fromDate:[NSDate date]];
    
    return ((selfYear == nowYear)&&(selfMonth == newMonth));
}
...
//    
//    NSString *allDate = [self.arrDisableDate componentsJoinedByString:@","];
//    if ([allDate containsString:[NSString stringWithFormat:@"%d",(int)[selectedDate timeIntervalSince1970]]]) {
//        if (self.delegate && [self.delegate respondsToSelector:@selector(calendarItem:didSelectedDate:)]) {
//            [self.delegate calendarItem:self didSelectedDate:selectedDate];
//        }
//    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if([self isSameDay:selectedDate date2:[NSDate date]]){
        if (self.delegate && [self.delegate respondsToSelector:@selector(calendarItem:didSelectedDate:)]) {
            [self.delegate calendarItem:self didSelectedDate:selectedDate];
        }
    }
